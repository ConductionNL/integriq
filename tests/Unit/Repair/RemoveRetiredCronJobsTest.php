...
use OCA\Integriq\Repair\RemoveRetiredCronJobs;
...
/**
 * @covers \OCA\Integriq\Repair\RemoveRetiredCronJobs
 *
 * @spec exclude No canonical spec covers the OCA\Integriq\Cron ->
 *  OCA\Integriq\BackgroundJob move; ADR-100 Decision 3 is an architecture record,
...
final class RemoveRetiredCronJobsTest extends TestCase {
	...
	public function testRemovesTheRegisteredRetiredClass(): void {
		...
		$this->assertContains('OCA\\Integriq\\Cron\\JobTask', $removed);
	...
	public function testAFailureOnOneClassDoesNotStopTheRest(): void {
		...
		$this->assertContains('OCA\\Integriq\\Cron\\StufZknRetryJob', $attempted, 'every class must be attempted even when each one throws');
