...
export default async function globalSetup(config: FullConfig): Promise<void> {
	...
	// Suppress the integriq product walkthrough for automated runs. On first
	// visit it mounts a modal spotlight tour whose full dim layer intercepts
	// pointer events, so every left-navigation click times out. The "seen"
	// marker is browser-local (`cn-walkthrough-seen:<appId>`), and a fresh
	// Playwright context re-triggers the tour every run.
	//
	...
	try {
		...
		await page.evaluate(() => {
			try {
				window.localStorage.setItem('cn-walkthrough-seen:integriq', '999.0.0')
			} catch (e) {
