...
Example — fetch the latest integriq SBOM:

```bash
curl -sL https://github.com/ConductionNL/integriq/releases/latest/download/sbom.cdx.json | jq .
```

Example — fetch the SBOM for a specific historical release:

```bash
curl -sL https://github.com/ConductionNL/integriq/releases/download/v1.0.0/sbom.cdx.json | jq .
```
