...
Example — fetch the latest openconnector SBOM:

```bash
curl -sL https://github.com/ConductionNL/openconnector/releases/latest/download/sbom.cdx.json | jq .
```

Example — fetch the SBOM for a specific historical release:

```bash
curl -sL https://github.com/ConductionNL/openconnector/releases/download/v1.0.0/sbom.cdx.json | jq .
```
