...
This page gets a working Integriq running on your own machine in two commands. You end with a connector you can point at a source system and watch feed the registry.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/integriq/development/integriq-compose.yaml
```
...
```bash
curl -s http://localhost:8602/status.php
```
...
| What | Where |
| --- | --- |
| **Integriq** | [http://localhost:8602/apps/integriq/](http://localhost:8602/apps/integriq/) |
| Admin interface | [http://localhost:8602](http://localhost:8602) — `admin` / `admin` |
...
| `integriq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing integriq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8602/apps/integriq/"
...
curl -s -u admin:admin "http://localhost:8602/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f integriq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f integriq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f integriq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
