...
description: Bring a pre-built Integriq configuration (sources, mappings, syncs, endpoints) into your instance in one go.
---

# Import a configuration bundle

Integriq ships configurations as JSON bundles — a single file (or set of files) that contains every Source, Mapping, Synchronization, Endpoint, Rule and Job that belongs to one integration. The **Import** screen lets an admin load such a bundle in one action, instead of recreating each object by hand.

## Goal

By the end you will have imported a configuration bundle from disk or from a URL, and confirmed that the bundled objects show up in the Integriq lists ready to use.

## Prerequisites

- You are an administrator on the Nextcloud instance, or your user has been granted the Integriq configuration role.
- You have a configuration bundle in hand — either a `.json` file from another Integriq instance, or a URL pointing at one (for example a GitHub raw URL).
...
1. From the Integriq navigation, click **Import**.
...
3. Review the preview. Integriq parses the bundle and lists what it will create or update — *N sources, M mappings, K synchronizations, …*. Confirm.
...
| Symptom | Fix |
|---|---|
| Import preview says `0 of everything` | The bundle is malformed JSON, or wraps the objects in an unexpected envelope — open the file and confirm it is an Integriq export, not a different app's format. |
