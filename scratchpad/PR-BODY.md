...
| 8 | `integriq.conduction.nl` (docs `url`, `static/CNAME`, `documentation.yml` `cname:`, `llms.txt`, manifest `documentationUrl`) | Live DNS + Pages CNAME. **Verified by curl: old → 200, `integriq.conduction.nl` → 000.** Moves with DNS, not with code. |
