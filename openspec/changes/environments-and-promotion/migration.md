...
## Current State
The `openconnector` OpenRegister register (`lib/Settings/integriq_register.json`,
...
— every Integriq entity is a generic OpenRegister object, not a
...
2. No change to `InitializeRegister.php` — it already merges every
   ...
   `importFromApp()`.
3. On `occ app:enable integriq` (fresh install) or `occ upgrade`
...
unreachable from the Integriq UI/API once the routes and controller
...
## Validation
- `occ app:enable integriq` on a fresh instance completes without
