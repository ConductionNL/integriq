# Context Brief: Endpoint and virtual desktop workspace connectors
...
**Lives at:** Adapters > Adapter-catalogus (Endpoint/Workspace) / Adapters
...
### Integration: Recast / Liquit
**demand: 55** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As an IT admin, I want mydash to integrate with Recast/Liquit, so that managed applications can be launched and provisioned directly from the dashboard. ACCEPTANCE CRITERIA:
1. GIVEN a Recast/Liquit environment is configured, WHEN an IT admin enters the connection credentials in mydash settings, THEN the integration status shows as connected and available apps are retrieved.
...
### Integration: VMware / Omnissa Horizon
**demand: 34** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As an IT admin, I want mydash to integrate with VMware Omnissa Horizon, so that virtual desktops are accessible from the dashboard. ACCEPTANCE CRITERIA:
