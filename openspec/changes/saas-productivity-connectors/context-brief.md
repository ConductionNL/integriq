# Context Brief: SaaS productivity and work management connectors

**App:** OpenConnector — API gateway and integration platform
**Spec:** saas-productivity-connectors
...
CAPABILITY: As a project manager, I want mydash integrated with Monday.com, so that tasks and project data sync between both platforms automatically. ACCEPTANCE CRITERIA:
1. GIVEN a connected Monday.com account, WHEN a board item is updated in Monday.com, THEN the change is reflected in mydash within 5 minutes.
2. GIVEN a task created in mydash, WHEN it is saved, THEN a corresponding item appears in the linked Monday.com board with matching title, status, and assignee.
3. GIVEN the Monday.com integration is enabled, WHEN the connection token expires or is revoked, THEN the project manager receives a notification prompting re-authentication.

### List View Integration
**demand: 54** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As a dashboard user, I want to view my data in a structured list format, so that I can scan and compare records efficiently. ACCEPTANCE CRITERIA:
1. GIVEN a dataset is loaded, WHEN the user switches to list view, THEN all records are displayed as rows with visible column headers.
2. GIVEN the list view is active, WHEN the user sorts by a column, THEN records reorder instantly to reflect the selected sort direction.
3. GIVEN the list view is active, WHEN the user selects one or more rows, THEN the selection is visually highlighted and available for bulk actions.

### DXP integration-first
**demand: 54** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As an IT admin, I want mydash to connect with external DXP systems via standard integration protocols, so that enterprise content and services are unified in one dashboard. ACCEPTANCE CRITERIA:
1. GIVEN a configured DXP endpoint, WHEN the IT admin saves the integration settings, THEN mydash retrieves and displays content from the external DXP without manual intervention.
2. GIVEN an active DXP integration, WHEN the source system publishes new content, THEN mydash reflects the update within the configured sync interval.
3. GIVEN invalid or expired DXP credentials, WHEN a sync is attempted, THEN mydash surfaces a clear error notification to the IT admin without disrupting other dashboard content.
