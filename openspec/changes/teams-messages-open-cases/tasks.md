...
- [x] Implement (`teams`: `HMAC <base64>` over the raw body under the
      base64-decoded secret, `hash_equals`, no timestamp, tolerance ignored
      with a warning)
- [x] Test (a good signature, a signature made under the literal secret, a
...
- [x] Implement (`receive`, `describe`, `reply`; the author as correspondent,
...
- [x] Test (a message naming a case number, one naming nothing, one matching
...
- [x] Implement (what an administrator creates in Teams, which URL to point
...
- [x] Test (the documented setup walked once against a real tenant, and the
      finding written down either way). NOT walked against a live tenant in
      this lane. The page says so in its own closing section rather than
