...
Integriq implements OpenRegister's pluggable `IntegrationProvider` contract
and registers a read-only, query-time `SynchronizationContractProvider` with
OR's `IntegrationRegistry`, so that every OR object Integriq has synced
...
The provider lives in Integriq (the owning app) per the fleet leaf rule
(hydra ADR-019). The implementation already ships on `development`; this delta
formalises the contract it fulfils (integriq#824).

## ADDED Requirements

### Requirement: SynchronizationContract integration provider (REQ-OCIP-001)

Integriq MUST provide a `SynchronizationContractProvider` in
...
icon `SyncOutline`, group `workflow`, required app `integriq`, and storage
strategy `query-time`.
...
`class_exists()` and a try/catch so that Integriq boots cleanly on an
