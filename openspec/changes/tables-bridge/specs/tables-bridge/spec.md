...
transport, and `MappingService` transformation pipeline. Tables is a soft
(feature-detected) runtime dependency: absent or disabled, `nextcloud-table`
simply does not exist as a source/target option.
...
- **Performance:** column metadata is fetched at most once per synchronization
  run (not per row); row writes/reads reuse the existing `CallService`
  rate-limit tracking already applied to any HTTP source.
- **Accessibility:** the table picker and column-mapping helper (sync-editor-ui
  delta) follow the same NcSelect/NcTextField accessibility conventions as
  the existing register/schema picker in `SyncConfigWidget.vue`.
- **Internationalization:** all new log/error messages and editor UI strings
  MUST be available in Dutch and English (hydra ADR-007); i18n keys MUST be
  authored in English per this workspace's `feedback_i18n-keys-english`
  convention.

## Acceptance Criteria

- [ ] `nextcloud-table` works as both a synchronization source and target,
      end to end, against a real Tables table in a dev environment.
- [ ] Contract `originId`/`targetId` mapping matches REQ-001/REQ-002 for
      create, update, and no-op (unchanged) cases.
- [ ] Column-type coercion covers text/number/datetime/selection per REQ-003,
      each with a passing and a failing-value test case.
- [ ] Feature detection hides the type and fails runs cleanly per REQ-004
      when Tables is disabled.
- [ ] Deletion only ever occurs through the shared `deleteInvalidObjects()`
      guard path (REQ-005) — no `nextcloud-table`-specific deletion bypass.
- [ ] A 401/403 from Tables aborts the run with no partial contract state
      (REQ-006).
- [ ] Editor table/column discovery endpoints (REQ-007) are consumed by the
      `sync-editor-ui` delta's table picker and column-mapping helper.

## Notes

- This spec deliberately targets the Tables `index.php/apps/tables/api/1/*`
  REST surface (not `ocs/v2.php/apps/tables/api/2/*`) for row/table/column
  CRUD — see design.md Decision 2 and discovery.md for the verified reason
  (v2 has no authenticated single-row GET/PUT/DELETE as of the schema
  fetched 2026-07-14).
- REQ-005 intentionally does not cite specific requirement ids from
  `sync-safety-guardrails` because that change is still at context-brief
  stage with no spec written yet; it describes the composition point
  (`deleteInvalidObjects()`) instead. When `sync-safety-guardrails` lands
  its spec, this requirement SHOULD be revisited to cite the concrete
  REQ ids it composes with.
- `usergroup`-type columns are read-only for this change (no write-side
  coercion rule in REQ-003); write support is deferred (see proposal Open
  Questions).
