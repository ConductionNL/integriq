...
- `openconnector_sources` (type column for source counts by type)
- `openconnector_call_logs` (status_code column for call counts by HTTP status)
- `openconnector_synchronizations` (total count of configured synchronizations)
- `openconnector_synchronization_logs` (result column for sync run counts by status)
- `openconnector_endpoints` (total count of registered endpoints)
- `openconnector_jobs` (total count of configured jobs)
- `openconnector_job_logs` (status column for job run counts by status)
- `openconnector_mappings` (total count of configured mappings)
- `openconnector_rules` (total count of configured rules)

## Implementation Status

### Implemented

- **MetricsController** (`lib/Controller/MetricsController.php`): Exposes `openconnector_info`, `openconnector_up`, `openconnector_sources_total` (by type), `openconnector_calls_total` (by status), `openconnector_synchronizations_total`, and `openconnector_synchronization_runs_total` (by status). Uses `IDBConnection` query builder with try/catch and zero-value fallbacks.
- **HealthController** (`lib/Controller/HealthController.php`): Returns `{"status": "ok"|"degraded"|"error", "checks": {"database": ..., "sources_table": ...}}`.
- **Route registration**: Both endpoints registered in `appinfo/routes.php`.

### Not Yet Implemented

- **`openconnector_endpoints_total`** (REQ-PROM-007): Total count is not yet emitted. `openconnector_endpoint_hits_total` deferred (requires EndpointService instrumentation).
- **`openconnector_jobs_total` and `openconnector_job_runs_total`** (REQ-PROM-008): No job metrics emitted.
- **`openconnector_mappings_total` and `openconnector_rules_total`** (REQ-PROM-009): No mapping/rule metrics emitted.
- **Critical source reachability** (REQ-PROM-010 scenario 5): Health check does not probe sources.
- **Admin authentication enforcement**: `#[AuthorizedAdminSetting]` not yet applied to controller methods.

## Standards & References

- **Prometheus text exposition format**: https://prometheus.io/docs/instrumenting/exposition_formats/
- **OpenMetrics specification**: https://openmetrics.io/
- **ADR-006 (company)**: Every app exposes `GET /api/metrics` (Prometheus, admin auth) and `GET /api/health` (JSON). Metric names carry `{app}_` prefix and MUST include `{app}_info`.
- **ADR-016 (company)**: Admin endpoints must use `#[AuthorizedAdminSetting(Application::APP_ID)]`.
- **OpenRegister HeartbeatController**: Reference implementation for health check pattern in the sibling app.
