...
### REQ-SPC-001: SaaS productivity and work-management connector adapters SHALL register through the integration registry per ADR-019
...
### REQ-SPC-002: Each SPC adapter manifest entry SHALL declare a fixed capability vocabulary across record-crud, event-subscribe, search-lookup, and bulk operations
...
### REQ-SPC-003: OAuth 2.0 SHALL be the default auth mode; service-account JWT and API key are explicit alternates only
...
### REQ-SPC-004: Search/lookup federation SHALL share the document-cms hit envelope, extended with `entityType`, `recordKey`, and `actorLabel` for record-shaped hits
...
### REQ-SPC-005: Event subscriptions SHALL normalise to CloudEvents and route through the standard event dispatcher per ADR-022
...
### REQ-SPC-006: Mutative bulk operations SHALL be opt-in per source AND gated by an admin-configured action authorisation per ADR-023
...
### REQ-SPC-007: Attachment bytes MUST stream through the adapter without local persistence; long-term storage SHALL go through docudesk per ADR-022
...
### REQ-SPC-008: Individual per-vendor adapters are explicitly out of scope for this spec — each adapter ships in its own `add-openconnector-{slug}-adapter` change
