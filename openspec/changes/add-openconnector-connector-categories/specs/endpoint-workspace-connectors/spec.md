...
### REQ-EWC-001: Endpoint and virtual-desktop / workspace connectors SHALL register through the integration registry per ADR-019
...
### REQ-EWC-002: Each EWC adapter manifest entry SHALL declare a fixed capability vocabulary scoped to session-enumeration, user-mapping, and audit-event ingestion
...
### REQ-EWC-003: User mapping SHALL go through a single declarative `UserMapping` shape — never per-app PHP
...
### REQ-EWC-004: Audit-event ingestion SHALL deposit events as CloudEvents per ADR-022, not as openconnector-local event tables
...
### REQ-EWC-005: Destructive operations (session-disconnect, device-wipe, force-logout) SHALL be opt-in per source AND gated by an admin-configured action authorisation per ADR-023
...
### REQ-EWC-006: Scheduled audit-event pulls SHALL run as OpenRegister `ScheduledWorkflow` records — no openconnector `TimedJob` per adapter
...
### REQ-EWC-007: Individual per-platform adapters are explicitly out of scope for this spec — each adapter ships in its own `add-openconnector-{slug}-adapter` change
