...
the remaining rule chain. Authorization follows ADR-023 (an app-wide action
gate) layered with a per-request approver-group membership check, since the
approver group is chosen per rule/synchronization, not app-wide. See
`design.md` Decisions 1-7 for the suspend/resume mechanics and the
declarative-vs-imperative notification split (ADR-031).
...
- **Performance:** the timeout sweep job MUST bound its per-run work to
  requests whose `expiresAt` has already passed (no full-table scan of
  resolved requests).
- **Accessibility:** Approve/Reject controls MUST be standard NC button
  components (not icon-only), satisfying WCAG AA per the project's NL
  Design System conventions.
- **Internationalization:** approval UI strings and the declarative
  ops-visibility notification's subject MUST be provided in both Dutch and
  English (hydra ADR-007), matching the pattern already used by
  `openconnector-notifications`.

## Acceptance Criteria

- [ ] An `approval` rule at `before` timing suspends the pipeline, persists
      an `approval_request`, and returns HTTP 202.
- [ ] The approver group receives an actionable notification; the
      `openconnector-ops` group receives a declarative visibility
      notification.
- [ ] Approval resumes the pipeline synchronously with the original
      (sensitive-header-stripped) context and completes normally.
- [ ] Rejection requires a comment and is fully audited.
- [ ] Timeout sweeping applies the configured fallback and is also
      enforced synchronously at approve/reject time.
- [ ] A user outside the specific request's `approverGroup` cannot approve,
      even if they pass the app-wide ADR-023 action-matrix check.

## Notes

- Synchronization-level batch gating (`requiresApproval`) is specified as a
  delta to `synchronization-engine` (see
  `specs/synchronization-engine/spec.md` in this change) rather than
  duplicated here; it reuses this capability's `approval_request` schema
  and authorization model with `synchronizationId` set instead of
  `endpointId`/`ruleId`.
- Replay of a `dead_letter` approval request against fresh source/target
  data is explicitly out of scope for this change (proposal.md Open
  Questions) — dead-lettered requests are discoverable and audited, not
  automatically retriable.
