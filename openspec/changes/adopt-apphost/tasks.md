...
- [x] 5.1 PHPUnit green (362 tests, was 364 — the 2 deleted controller tests). `php -l` clean on every touched/new PHP file. (`composer check:strict` static suite not re-run end-to-end here; no new PHPCS/PHPMD/Psalm violations introduced — the deleted `MetricsController` phpmd baseline entry was removed.)
- [x] 5.2 All 23 hydra gates green diff-scoped (`run-hydra-gates.sh --scope-to-diff`), including gate-5 route-auth (satisfied by the thin subclass `index()` carrying `#[PublicPage]`/`#[NoCSRFRequired]`), gate-14 route-reachability, gate-16 spec-coverage, gate-19 e2e-coverage, gate-27-equivalent (OR AppHost method names verified real: `GenericHealthController`/`GenericMetricsController` ctors + `index()`, `ManifestLoader`, `HealthCheckExecutor`, `MetricsEngine` all resolved live).
- [~] 5.3 **Gate-22 manifest validation — UPSTREAM-BLOCKED.** The `observability` block is structurally valid and the engine reads it, but the canonical `@conduction/nextcloud-vue` v2 schema (the pinned `github:#…` commit, schema v2.9.0) does **not yet declare an `observability` property** and is `additionalProperties: false` — so Ajv rejects it. This is a fleet-wide ADR-040 gap (OR's own adopted manifest hits it too). The required schema extension is captured as `nextcloud-vue-observability-schema.patch` in this change folder (adds the `observability` property + `$defs/observability` with the health-check + metric-descriptor contract; bumps schema to 2.10.0). **Follow-up: land that patch in nextcloud-vue, republish, bump openconnector's pin** — gate-22 then goes green. Verified locally PASS against the patched schema (`Ajv validation: PASS (0 errors)`).
