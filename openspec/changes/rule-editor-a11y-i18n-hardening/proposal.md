...
`src/modals/Rule/EditRule.vue` — the rule-condition/action editor modal, openconnector's
...
1. **Two hardcoded, untranslated English literals ship as user-visible button text**:
   ...
   actions are plain text nodes, not `{{ t('openconnector', 'Cancel') }}` /
   `{{ t('openconnector', 'Save') }}`. Every other modal in this app wraps these same two
   words in `t()` — e.g. `RuleDetailPage.vue:54` (`{{ saving ? t('openconnector',
   'Saving…') : t('openconnector', 'Save') }}`), `AddEndpointRule.vue:41`
   (`{{ t('openconnector', 'Save') }}`). Because `tests/l10n/check-l10n.js` only checks
...
2. **Two icon-only `<NcButton>` instances have no accessible name.** The "remove"
   ...
   delete buttons) correctly carries `:aria-label="t('openconnector', '...')"`, so this
   is a real, local regression pattern, not house style.

## What Changes

- Wrap `Cancel` (`EditRule.vue:599`) and `Save` (`EditRule.vue:619`) in
  `{{ t('openconnector', 'Cancel') }}` / `{{ t('openconnector', 'Save') }}`, matching
...
- Add `:aria-label="t('openconnector', 'Remove property')"` to the two icon-only
