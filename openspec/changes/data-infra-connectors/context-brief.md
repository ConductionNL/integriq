# Context Brief: Infrastructure and domain data source connectors

**App:** OpenConnector — API gateway and integration platform
...
**Lives at:** Adapters > Adapter-catalogus (Infra/Data) + Bronnen (DB/S3/MQ sub-types) / split

**Rationale:** Catalogue listing + actual bron-types appear in Bronnen filter  
...
### Integration: Vilans KICK
**demand: 55** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As a care professional, I want mydash integrated with Vilans KICK, so that client data syncs automatically between systems. ACCEPTANCE CRITERIA:
1. GIVEN a valid Vilans KICK connection is configured, WHEN a client record is updated in KICK, THEN the updated data is reflected in mydash within the defined sync interval.
2. GIVEN a care professional is logged into mydash, WHEN they view a client profile, THEN data sourced from Vilans KICK is visibly present and accurate.
3. GIVEN the KICK integration is active, WHEN the connection fails or credentials expire, THEN an alert is shown to the administrator and data sync halts gracefully without data loss.

### [Feature]: InfluxDB Integration
**demand: 54** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As a dashboard operator, I want to connect mydash to an InfluxDB data source, so that time-series metrics are visualized in real time. ACCEPTANCE CRITERIA:
1. GIVEN valid InfluxDB connection credentials, WHEN the operator saves the data source configuration, THEN mydash confirms the connection and lists available buckets/measurements.
2. GIVEN a configured InfluxDB source, WHEN a dashboard panel is created with a Flux or InfluxQL query, THEN the panel renders the queried time-series data without error.
3. GIVEN an InfluxDB source with authentication failure, WHEN the operator tests the connection, THEN mydash displays a descriptive error indicating the credential or endpoint issue.

### Dovecot Pro integration
**demand: 54** (1 tender mentions) | Category: integration | **Coverage: openconnector → `openconnector`**

CAPABILITY: As an IT admin, I want Dovecot Pro integration, so that enterprise email storage and delivery is managed seamlessly within mydash. ACCEPTANCE CRITERIA:
1. GIVEN Dovecot Pro credentials are configured, WHEN the admin saves the integration settings, THEN mydash connects to Dovecot Pro and reports a successful connection status.
2. GIVEN the Dovecot Pro integration is active, WHEN a user's mailbox is provisioned or deprovisioned, THEN the change is reflected in Dovecot Pro without manual intervention.
3. GIVEN Dovecot Pro is unreachable, WHEN the integration attempts a sync or mail operation, THEN mydash displays a clear error message indicating the connection failure.
