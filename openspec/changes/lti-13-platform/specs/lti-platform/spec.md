# lti-platform — Delta: LTI 1.3 / LTI Advantage adapter (Platform + Tool roles)

## Purpose

Establishes a generic LTI 1.3 + LTI Advantage adapter in OpenConnector so any
Conduction app can either consume an external LTI tool (this instance acts as
**Tool**, launched by an external Platform such as a Moodle-based state
platform) or expose its own content as an LTI tool to an external Platform
(this instance acts as **Platform**, launching an external Tool). The
adapter covers OIDC third-party-initiated login, signed-JWT launch
validation, Deep Linking 2.0, Assignment & Grade Services (AGS), and Names &
Role Provisioning Services (NRPS), delivered per ADR-017 Rule 1 (Adapters
catalogue entry) with tenant-wide signing-key/registration management under
Rule 3/7 (Beheer > Authenticatie), and dedicated public protocol routes
mirroring the `dso-omgevingsloket` precedent.
