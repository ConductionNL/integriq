# eudi-wallet-credential-issuance — Delta: OpenID4VCI credential issuer adapter

## Purpose

Establishes a new `eudi-wallet-credential-issuance` capability: an OpenID4VCI
(pre-authorized code flow) Credential Issuer surface that turns an
already-assembled, already-signed W3C Verifiable Credential payload
(EDCI/ELM-profiled diploma or Open Badges 3.0 assertion, handed over by a
consuming app such as scholiq) into a wallet-consumable credential offer,
serves the OpenID4VCI issuance endpoints an EUDI wallet calls, and publishes
a revocation status list. Delivered per ADR-017 Rule 1 (Adapters catalogue
entry, no top-level menu) with the issuer signing-key lifecycle under
*Beheer > Authenticatie* (Rule 3/7, the same sanctioned split as
`digid-eherkenning-auth-adapter`). Inbound authority for creating an offer is
gated by the existing `consumer-management` (REQ-CON-001) and
`authorization-jwt` (REQ-001) mechanisms; revocation callbacks reuse the
existing `webhook-signing` (REQ-WHS-001/002) HMAC scheme. Driven by insight
1150 (Regulation (EU) 2024/1183, member-state EUDI wallets by end of 2026).
