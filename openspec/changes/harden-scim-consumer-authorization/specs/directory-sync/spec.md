...
- **Reads are not constrained by REQ-DS-003, deliberately.** `GET /Groups` answers
  each group's complete membership, and the page-size clamp bounds how many groups
  answer rather than how much data — so `?count=1` returns one group carrying every
  uid and display name in it. For an `everyone`-style group that is the account
  estate, reachable with any valid consumer key.
  Trimming the response is the wrong axis: RFC 7643 §4.1.2 makes `Group` the
  authoritative membership resource — *"Since this attribute has a mutability of
  `readOnly`, group membership changes MUST be applied via the `Group` Resource"* —
  and §8.7.1 gives `Group.members` `returned: "default"`, so a conformant provider
  answers it unless the client narrows the request. Integriq already accepts the
  write side of that resource. RFC 7644 paginates resources (§3.4.2.4) and selects
  attributes (§3.4.2.5) but has no pagination *within* a multi-valued attribute, so
  it offers no bound for one large group either.
  The bound is therefore an authorization question — which consumer key may read
  which accounts and groups — tracked as ConductionNL/integriq#2112, with the
  protocol half (paging, attribute selection, discovery) as
  ConductionNL/integriq#2116. Pinned meanwhile by
  `testAGroupsFullMembershipIsReturned()`.
