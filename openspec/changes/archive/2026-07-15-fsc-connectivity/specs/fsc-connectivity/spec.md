...
OpenConnector gains FSC (Federatieve Service Connectiviteit — the VNG/Common
Ground standard that replaced NLX in 2025) connectivity so sibling apps can
reach another organisation's published service through the standard Dutch
inter-municipality directory + service-contract + mTLS-tunnel model, without
embedding a directory/mTLS client of their own. Per ADR-022 integrations
live in openconnector, not as nc-vue leaves and not per-app. A narrow
`FscConnectivityProviderInterface` (`resolveService`, `call`) lets a
compatible alternative transport (e.g. a real Outway-backed binding) be
added later; this change ships a `log`/sandbox binding and a `rest` binding
(`FscDirectoryClient`) so the whole resolve/call path is demonstrable
end-to-end without a live FSC Directory/Outway/Inway.
