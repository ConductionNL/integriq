...
outcome — without embedding a PSP client of their own. The connector is
delivered as a source/adapter configuration in the same family as
`peppol-access-point-connector` and `psd2-ais-bank-feed-connector`: it
resolves PSP credentials through the OpenRegister credential broker
(ADR-007) and integrates via CloudEvents (`events-cloudevents`) and the
openconnector HMAC webhook-signature pattern (`webhook-signing`) — not a
parallel framework. Mollie is the first real binding (dominant NL PSP, iDEAL
the dominant NL method). Per ADR-022 the PSP integration lives here; leaf apps
(e.g. shillinq) consume it via its REST surface and its CloudEvents.
...
- **Performance:** payment creation SHOULD time out and return a descriptive
  502 rather than hang the request; the webhook's re-fetch call is on the
  synchronous critical path and MUST also time out rather than hang.
- **Accessibility:** any PSP-source configuration UI reuses existing
...
- [ ] `POST /api/payments` creates a payment and returns a checkout URL,
  matching shillinq's `createPayment()` request envelope
- [ ] `log` provider drives create + status with no network call or secret
- [ ] `POST /api/payments/webhook` verifies the signature and never trusts an
  inbound status claim — it always re-fetches
- [ ] Provider status is mapped to `authorized|captured|failed|voided` and
  `nl.conduction.payment.status` is emitted on every actual outcome change
- [ ] A replayed webhook for an unchanged outcome does not double-emit
  (idempotent)
- [ ] PSP credentials are resolved via `credentialRef`; none appear in
  config/logs

## Notes

- Reuses `events-cloudevents` (emission), the openconnector webhook-signature
  HMAC pattern (`WebhookSignatureService`, as used by
  `peppol-access-point-connector`), `BrokeredCallService`/`CredentialBrokerService`
  (ADR-007), and `source-management` mock-mode.
- Cross-app contract owner: this spec. shillinq's
  `MolliePaymentAdapterInterface` production binding (a follow-up change, not
  built here) targets REQ-LPP-001 and observes REQ-LPP-004/REQ-LPP-005.
- Wero (iDEAL's scheduled end-2027 successor) is explicitly NOT implemented
  here; REQ-LPP-002 requires the interface shape to accommodate it later
