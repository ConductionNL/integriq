...
**Scope**: openconnector
**OpenSpec changes**:
- `endoflife-date-source` (this change)

## Purpose

Ship a ready-to-run source preset for the public
[endoflife.date](https://endoflife.date/api) lifecycle API — a seeded
`source`, two new schemas (`eolProduct`, `eolCycle`) declared in the
existing `openconnector` register, and a curated set of scheduled
`Synchronization`/`Mapping`/`Job` triples — so a fresh OpenConnector
install can pull real, live end-of-life/support-window data for a starter
set of technologies with zero operator setup and no credential. Built
entirely on the existing `source-management`, `synchronization-engine`,
`mapping-and-search`, `job-scheduling`, and `connector-catalog`
capabilities (none modified). See `openconnector-register-schema` for the
single-register-per-app convention this change's schemas follow.
