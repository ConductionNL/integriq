...
OAuth flow, or consent record lives in shillinq (REQ-BC-002/003). Built on the
existing `source-management`, `job-scheduling`, and `events-cloudevents`
capabilities — not a parallel framework.
...
- **Performance:** the sync job MUST page aggregator transaction results and
  bound each `bankfeed_batch` so a large backfill does not exhaust memory; a
  per-account pull MUST time out rather than hang the sweep.
- **Accessibility:** any PSD2-source configuration UI reuses existing
...
- [ ] `Psd2AggregatorProviderInterface` with `log` + `rest` bindings; `rest` brokers its token
- [ ] `POST /api/psd2/connect` returns the bank SCA redirect URL; `GET /api/psd2/callback` finalises consent
- [ ] Account discovery records IBAN/BIC/currency idempotently, no token exposed
- [ ] `BankfeedSyncJob` pulls per-connection and emits `nl.conduction.bankfeed.transactions.synced` with `batchUri`
- [ ] Consent-lifecycle events (`granted`/`expiring`/`revoked`) are emitted
- [ ] Tokens resolved via `credentialRef`; none appear in config/objects/logs

## Notes

- Cross-app contract: shillinq `bookkeeping-bank-connectors`
  (`/home/.../shillinq/openspec/specs/bookkeeping-bank-connectors/spec.md`)
  REQ-BC-001 consumes this per ADR-022; field names (`consentReference`,
  `consentExpiresAt`, `bankAccountIban`, `aggregatorSourceSlug`, `aggregator`)
  are aligned with REQ-BC-002. shillinq's SCA-renewal action (REQ-BC-006)
  redirects to `POST /api/psd2/connect`; its scheduled workflow (REQ-BC-004)
  materialises CAMT.053 from the `bankfeed_batch` this connector emits.
- Reuses `job-scheduling` (`BankfeedSyncJob` as a `JobTask` action + cron
  sweep), `events-cloudevents` (event emission), `source-management`
  (source/config + mock mode), and `configuration-export-import` (redaction).
