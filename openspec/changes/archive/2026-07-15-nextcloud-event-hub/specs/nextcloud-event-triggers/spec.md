...
## Purpose
Nextcloud core and its bundled/optional apps (files, calendar, Tables, Forms) emit in-process PHP events
that today go nowhere outside NC's own `workflow_engine`/`webhook_listeners`. This capability registers
OpenConnector `IEventListener`s directly against those events — no HTTP hop, no polling — and normalizes
each into the same CloudEvents `event` envelope OpenRegister object-lifecycle events already use (see
`events-cloudevents` REQ-004), so every existing filter/deliver/retry/sign/dead-letter mechanism applies to
Nextcloud-native events with zero duplicated code. It also defines the non-admin self-service subscription
path — default-deny per-event-family actions in the existing ADR-023 action matrix (`ActionAuthService`,
`lib/actions.seed.json`, `ActionAuthMatrix.vue`) — that lets ordinary users subscribe to event families an
administrator has explicitly granted their NC group, without needing an admin to create every subscription
by hand.
...
- **Performance:** Listener `handle()` methods MUST return within typical NC event-dispatch budget
  (no blocking network calls inside the listener itself — `EventService::handleNextcloudEvent` persists and
  hands off to the existing `processEvent`/delivery pipeline, which already runs outbound HTTP
  asynchronously relative to the triggering NC request per existing `events-cloudevents` behaviour).
- **Accessibility:** Self-service subscription UI additions reuse existing NcSelect/NcCheckboxRadioSwitch
  components already validated for WCAG AA in this app (`consumer-management` UI baseline); the admin
  allow-list surface is the pre-existing `ActionAuthMatrix.vue` editor, unchanged.
- **Internationalization:** All new UI strings (self-service subscription error/success states) MUST ship
  with `nl` and `en` translations, English source keys (hydra ADR-007). The action-matrix editor's existing
  strings are unchanged.

## Acceptance Criteria

- [ ] File, calendar, Tables (feature-detected), and Forms (feature-detected) listeners are registered and
      each produces a correctly-typed, correctly-sourced `event` record when its NC event fires.
- [ ] A non-admin's `subscribe()` call for an NC-native type is rejected by default (seeded `["admin"]`)
      and succeeds only once an admin grants their NC group both `event.subscribe` and the relevant
      `event.subscribe-nextcloud-<domain>` action via the existing action-matrix editor.
- [ ] Tables/Forms listeners register only when their source app is installed and enabled; no error is
      logged when absent.
- [ ] A malformed/unexpected DAV calendar event shape is logged and skipped, never thrown into the NC event
      dispatcher.

## Notes

- Tables/Forms event class names used above (and in `design.md`/`tasks.md`) are the best-effort names
  identified during `discovery.md`'s research and are explicitly flagged TENTATIVE — the implementing task
  MUST confirm exact class names/payload shapes against a live instance before merging listener code that
  references them by FQCN.
- This spec intentionally does NOT modify the pre-existing coarse-action authorization posture of
  `subscribe()`/`updateSubscription()` for non-NC-native types, and does NOT add per-OBJECT ownership
  checks (the residual gap behind the `events-cloudevents` spec's REQ-005 Notes: ADR-023 action RBAC gates
  who may call the endpoint, not which subscription a caller may touch by UUID). That remains open, tracked
  separately — see `nextcloud-event-hub` `proposal.md` Risk 2 and `design.md` Decision 6.
