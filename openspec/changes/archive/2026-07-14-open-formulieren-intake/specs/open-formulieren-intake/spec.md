...
OpenConnector gains a bridge from Open Formulieren (the NL government's
de-facto standard no-code form builder) submissions into fleet case intake,
feeding OpenRegister's shipped semantic-object-handoff engine (`ns#Case`,
ADR-051) rather than a point-to-point bridge to any one app. Per ADR-022,
integrations live in openconnector, not as nc-vue leaves and not per-app.
