...
- **WHEN** a `npsLv01` request queries by geslachtsnaam "Moulin" (partial match) and the adapter searches OpenRegister
- **THEN** it returns all matching persons in a multi-record `npsLa01` response, respecting the `maximumAantal` parameter if specified

#### Scenario: Scope element limits returned fields
- **WHEN** a `npsLv01` request includes a `scope` element requesting only BSN and naam fields and the adapter builds the response
- **THEN** only the requested fields are included in the `npsLa01` response, with unrequested fields omitted (not set to `geenWaarde`)

#### Scenario: Unknown BSN returns empty response
- **WHEN** a `npsLv01` request specifies a BSN that does not exist in OpenRegister and the adapter searches
- **THEN** it returns an empty `npsLa01` response (zero records) with correct stuurgegevens but no error

#### Scenario: Malformed request returns Fo01 fault
- **WHEN** a `npsLv01` request has malformed XML or missing required StUF elements and the adapter validates the request
- **THEN** it returns a StUF `Fo01` fault message with diagnostic information including the specific validation error

### Requirement: StUF-BG Field Mapping (REQ-STUF-002)
...
#### Scenario: Custom field name mapping is honored
- **WHEN** a municipality uses a custom field name "achternaam" instead of "geslachtsnaam" in their OpenRegister schema and updates the StUF-BG mapping to map `geslachtsnaam` -> `achternaam`
- **THEN** the adapter uses the custom mapping for all subsequent `npsLa01` responses

#### Scenario: Date transformed to StUF format
- **WHEN** an OpenRegister person has a geboortedatum stored in ISO 8601 format ("1990-05-15") and the adapter maps to StUF-BG
- **THEN** the date is transformed to StUF format `YYYYMMDD` ("19900515") using the date transformation rule

#### Scenario: Nested address mapped into element hierarchy
- **WHEN** a person's verblijfsadres is stored as a nested object in OpenRegister and the adapter maps to StUF-BG
- **THEN** the nested fields are correctly placed into the StUF-BG `verblijfsadres` element hierarchy

### Requirement: StUF-BG Address Query (adrLv01/adrLa01) (REQ-STUF-004)
...
#### Scenario: Query without huisnummer returns all matches
- **WHEN** the BAG register contains 3 addresses matching postcode "1234AB" and the query does not specify huisnummer
- **THEN** all 3 addresses are returned in the `adrLa01` response

#### Scenario: No matching address returns empty response
...
- **WHEN** the external StUF-BG service returns a `Fo01` fault message and the adapter processes the response
- **THEN** the SOAP fault is mapped to a CallLog entry with HTTP-equivalent status (e.g., Fo01 "not found" -> 404, Fo01 "unauthorized" -> 401) and descriptive error details

#### Scenario: Multiple records extracted as separate objects
- **WHEN** the external StUF-BG service returns multiple person records and the adapter processes the response
- **THEN** each person is extracted as a separate JSON object and can be stored as individual OpenRegister objects via SynchronizationService

#### Scenario: Nightly synchronization with change detection
- **WHEN** a StUF-BG synchronization is configured to pull person data nightly and the sync job runs
- **THEN** SynchronizationService queries the external StUF source, maps responses to OpenRegister objects using the configured field mapping, and creates/updates records with change detection
...
- **THEN** it maps the StUF fields (zaakidentificatie, omschrijving, startdatum, zaaktype, status) to OpenRegister properties, creates the zaak object in Procest's register, and returns a `Bv03` bevestiging message with the zaakidentificatie

#### Scenario: Update existing zaak via zakLk01
- **WHEN** a legacy system sends a `zakLk01` with an existing zaakidentificatie and the adapter receives the update message
- **THEN** it finds the existing zaak in OpenRegister, updates the modified fields, and returns a `Bv03` bevestiging

#### Scenario: Retrieve zaak with statussen and documenten
- **WHEN** a legacy application sends a `zakLv01` request for zaak "ZAAK-2024-001" and the adapter processes the query
- **THEN** it retrieves the zaak from OpenRegister with its statussen and linked documenten, and returns a `zakLa01` response with the complete zaak data in StUF-ZKN format

#### Scenario: Invalid zakLk01 returns Fo03
- **WHEN** the `zakLk01` message contains invalid data (e.g., missing required zaaktype) and validation fails
- **THEN** the adapter returns a `Fo03` foutmelding with the specific validation error

#### Scenario: Query by zaaktype and date range
- **WHEN** a `zakLv01` request queries by zaaktype and date range and the adapter processes the query
- **THEN** it filters OpenRegister objects by the zaaktype and startdatum range and returns matching zaken in the `zakLa01` response

### Requirement: StUF-ZKN Document Linking (edcLk01) (REQ-STUF-022)
...
#### Scenario: Document metadata stored alongside content
- **WHEN** an `edcLk01` message contains document metadata (titel, auteur, creatiedatum, vertrouwelijkheidaanduiding) and the adapter processes the message
- **THEN** the metadata is stored alongside the document in Nextcloud Files and linked as zaak-document properties

#### Scenario: edcLk01 for missing zaak returns Fo03
- **WHEN** an `edcLk01` references a zaak that does not exist and the adapter validates the reference
- **THEN** it returns a `Fo03` fault message indicating the zaak was not found
...
- **THEN** the adapter sends a `zakLv01` with zaaktype filter, parses the `zakLa01` response, and maps each zaak to an OpenRegister object

#### Scenario: genereerZaakIdentificatie precedes zakLk01
- **WHEN** the legacy system supports `genereerZaakIdentificatie` and a workflow needs to create a zaak in the legacy system
- **THEN** the adapter first requests a zaak ID via `genereerZaakIdentificatie` and uses it in the subsequent `zakLk01`

#### Scenario: Linked document references stored as eigenschappen
- **WHEN** the `zakLa01` response includes linked document references and the adapter processes the zaak data
- **THEN** document references are stored as zaak-eigenschappen with their StUF document IDs, enabling subsequent `edcLv01` retrieval

### Requirement: WSDL and XSD Bundling (REQ-STUF-040)
...
#### Scenario: Bundled WSDL used when no external URL configured
- **WHEN** a StUF-BG source is configured and SOAPService.setupEngine() initializes the SOAP client
- **THEN** the bundled WSDL is used if no external WSDL URL is specified in the Source configuration

#### Scenario: Extended WSDL URL overrides bundled version
- **WHEN** a municipality uses StUF-ZKN 3.10e (extended version) and configures the source
- **THEN** they can specify the extended WSDL URL in the Source configuration, overriding the bundled 3.10 version
...
- **THEN** it includes the correct namespace declarations: `xmlns:StUF="http://www.egem.nl/StUF/StUF0301"`, `xmlns:BG="http://www.egem.nl/StUF/sector/bg/0310"`, etc.

#### Scenario: Response validates against StUF XSD
- **WHEN** a legacy consumer validates responses against the StUF XSD and the adapter sends a response
- **THEN** the XML validates against the schema with zero namespace errors

#### Scenario: Non-standard prefix resolved by URI
- **WHEN** the adapter processes an incoming message with a non-standard namespace prefix and it parses the XML
- **THEN** it resolves elements by namespace URI (not prefix) ensuring interoperability with different StUF implementations

### Requirement: Stuurgegevens Population (REQ-STUF-042)
...
#### Scenario: Outbound query stuurgegevens use Source config
- **WHEN** the adapter sends an outbound `npsLv01` query to a BRP system and stuurgegevens are populated
- **THEN** `zender` contains the municipality's OIN (from Source configuration) and `ontvanger` contains the BRP system's code

#### Scenario: Missing zender config blocks message
- **WHEN** the configured zender code is missing in the Source settings and the adapter attempts to send a message
- **THEN** it returns an error indicating that StUF stuurgegevens configuration is incomplete, preventing malformed messages
...
- **THEN** the `voorvoegsel` element includes `StUF:noValue="geenWaarde"` with empty content

#### Scenario: waardeOnbekend stored with annotation
- **WHEN** the adapter receives a StUF-BG response with `geboortedatum StUF:noValue="waardeOnbekend"` and it maps to an OpenRegister object
- **THEN** the field is stored as null with a metadata annotation indicating "waardeOnbekend"

#### Scenario: nietOndersteund field omitted on map
- **WHEN** the adapter receives a StUF field with `noValue="nietOndersteund"` and maps to OpenRegister
- **THEN** the field is omitted from the stored object (not supported by the source system)

#### Scenario: Scope-excluded field omitted from XML
- **WHEN** the scope element in a `npsLv01` request does not include a specific field and building the response
- **THEN** that field is excluded entirely from the XML (different from `noValue` which explicitly communicates absence)
...
- **THEN** it contains entries like `{"stufPath": "inp.bsn", "registerProperty": "burgerservicenummer", "direction": "bidirectional", "transformation": null}` for each mapped field

#### Scenario: Custom extension field included both directions
- **WHEN** a municipality has a custom StUF extension adding a "klantbeeld-id" field and the admin adds a custom mapping entry
- **THEN** the adapter includes this field in both inbound and outbound StUF messages

#### Scenario: Date transformation applied both directions
- **WHEN** the mapping includes a date transformation (StUF `YYYYMMDD` to ISO 8601) and the adapter maps a geboortedatum field
- **THEN** the value is transformed in both directions: "19900515" (StUF) <-> "1990-05-15" (OpenRegister)

#### Scenario: inbound-only mapping excluded from responses
- **WHEN** a mapping entry has direction "inbound-only" and the adapter processes an inbound StUF message
- **THEN** the field is mapped from StUF to OpenRegister but is NOT included when generating outbound StUF responses

#### Scenario: ZGW-zaak mapping applied on zakLk01
- **WHEN** the default ZGW-zaak mapping is loaded and a `zakLk01` message arrives
- **THEN** StUF-ZKN fields (zaakidentificatie, omschrijving, startdatum, einddatum, zaaktype, status) are mapped to their OpenRegister equivalents using the pre-seeded mapping

### Requirement: Value Transformations (REQ-STUF-053)
...
#### Scenario: Code list lookup both directions
- **WHEN** a code list transformation maps geslachtsaanduiding and StUF sends "V"
- **THEN** the OpenRegister object stores "vrouw", and vice versa for outbound messages

#### Scenario: Concatenation combines name parts
...
- **THEN** the Source entity is created with StUF-specific configuration in the `configuration` JSON field

#### Scenario: Connectivity test reports result
- **WHEN** a StUF source is configured and the administrator tests connectivity
- **THEN** the adapter sends a minimal SOAP request (e.g., a ping or capability query) and reports success/failure with diagnostic details

#### Scenario: WS-Security headers added automatically
- **WHEN** a StUF source is configured with WS-Security and an n8n workflow queries the source
- **THEN** the WS-Security headers are automatically added to the SOAP envelope by AuthenticationService

#### Scenario: Health check surfaces SSL error detail
