...
OpenConnector gains a KISS (Klantinteractie Servicesysteem) bridge so
sibling apps (e.g. procest's native KCC module) can exchange klantcontacten
with a municipality's KISS deployment without embedding a client of their
own. KISS is the open-source Common Ground KCC component built by Utrecht +
Dimpact (live, co-developed by ~8 municipalities) and the reference KCC
integration target for zaaksystemen — it implements the VNG "Klantinteracties
API" (the OpenKlant 2.x model). Per ADR-022 integrations live in
openconnector, not as nc-vue leaves and not per-app. A narrow
`KlantinteractiesProviderInterface` (list/create/link) lets a compatible
alternative backend implement the same contract later; this change ships a
`log`/sandbox binding and a `rest` binding (`KlantinteractiesClient`) so the
whole pull/push path is demonstrable end-to-end without a live credential.
