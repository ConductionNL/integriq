...
2. **Adapter metadata registry** (PHP-side, single source of truth) — a new `CatalogRegistryService` in OpenConnector that assembles catalog entries from (a) the existing OR-side `IntegrationRegistry` for the 4 registered category adapters, (b) a small hand-written descriptor list for PDOK/Digikoppeling/Berichtenbox/DSO (not currently in any registry), and (c) the `register.d/*.json` seeded-source fragments, materialised into `catalog_item` OpenRegister objects by a repair step so the Catalog page can be a standard register/schema-backed `index` page.
