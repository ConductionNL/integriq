# Comprehensive Tests Specification

**Status**: in-progress
**Scope**: openconnector
**OpenSpec changes**:
- `openconnector-comprehensive-tests`

## Purpose

This spec defines the test infrastructure requirements for OpenConnector after the A→D2
migration chain. It establishes what the PHPUnit, Newman, and Playwright suites MUST cover,
what coverage thresholds MUST be enforced in CI, and what the migration round-trip test
MUST verify. This is the verification gate that asserts chains A through D2 actually work
as a system.

ADR references (all qualified `(hydra)` vs `(local)` to avoid the namespace collision since both repos have an ADR-008 and ADR-009):
- **(hydra) ADR-008** — fleet-wide testing standards
- **(local) ADR-005** — Source/Synchronization/Contract triad
- **(local) ADR-007** — Source credentials currently stored plaintext (no encryption layer)
- **(local) ADR-008** — Endpoint polymorphic `targetType/targetId` dispatch (preserve under chain C rewrite)
- **(local) ADR-009** — Multi-platform DB (MySQL + Postgres) via Nextcloud abstraction
...
- **Performance:** PHPUnit suite MUST complete in < 2 min; Newman in < 3 min; Playwright
  regression in < 10 min (wall time, CI environment)
- **Accessibility:** not applicable (test code only)
- **Internationalization:** test code is English-only; no user-facing strings added
  (hydra ADR-007 i18n not applicable — test code is dev-facing and English-only)

## Acceptance Criteria

- [ ] `composer test:coverage` exits 0 with ≥ 80% line coverage and ≥ 70% branch coverage
- [ ] `newman run tests/postman/openconnector.postman_collection.json --environment tests/postman/openconnector.postman_environment.json` exits 0 with 0 failures
- [ ] `npx playwright test --project regression` exits 0 with 0 failures
- [ ] `tests.yml` CI workflow is present and all three jobs are merge-blocking
- [ ] `tests/Helpers/ObjectServiceMockBuilder.php` exists
- [ ] All 9 service test files and 15 DTO test files exist under `tests/Unit/`
- [ ] `tests/e2e/migration-round-trip.spec.ts` exists
- [ ] `playwright.config.ts` has a `regression` project with `workers: 4`

## Notes

- Chain E MUST NOT modify any file under `lib/`, `src/`, or `appinfo/`.
- The `docs-screenshots.spec.ts` spec is excluded from the `regression` project to keep
  CI fast (opt-in via `--project docs-capture` only).
- ADR-009 (multi-platform DB): Postgres only in first iteration; MySQL matrix deferred.
- ADR-007 (plaintext credentials): the Newman collection MUST use placeholder credentials
  (`admin`/`admin`) and MUST NOT commit real API keys or passwords.
- ADR-031 (declarative-vs-imperative): the 80/70 coverage thresholds are declarative
  policy; the PHPUnit test code that produces the measurements is imperative. Both are
