# digikoppeling-adapter — Delta: Digikoppeling transport (WUS + ebMS2 + Grote Berichten)

## Purpose

Establishes the Digikoppeling transport adapter reserved by ADR-017: a catalogue
entry in *Adapters* that carries StUF/ZGW message content over the three
Logius-defined Digikoppeling profiles — WUS (synchronous, WS-Security-signed),
ebMS2 (asynchronous reliable messaging), and Grote Berichten (large payloads) —
with PKIoverheid certificate custody through the OpenRegister credential broker.
The adapter provides transport only; StUF/ZGW message *bodies* remain owned by
the StUF/ZGW content services.
