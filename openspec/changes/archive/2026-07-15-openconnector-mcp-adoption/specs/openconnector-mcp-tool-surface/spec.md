# Spec: openconnector-mcp-tool-surface (delta)

## ADDED Requirements

### Requirement: REQ-MCP-101 — OpenConnector's MCP tool surface MUST be declared on the schema via the `x-openregister-mcp` dialect and MUST NOT be hand-written in PHP

OpenConnector MUST NOT ship an MCP tool provider, an `IMcpToolProvider` implementation, or any
hand-coded tool descriptor (ADR-063). The entire openconnector tool surface MUST be **derived**
by OpenRegister from a per-schema `configuration["x-openregister-mcp"]` block, producing tool
ids of the form `openconnector.{schema}.{verb}`. The declaration MUST live in
`lib/Settings/openconnector_register.json` alongside the schema it describes. Exposure MUST
...
- **AND** the only artifact changed is `lib/Settings/openconnector_register.json`
...
- **THEN** no `openconnector.{that-schema}.{verb}` tool exists
...
Exactly **8** of openconnector's 27 schemas MUST carry the dialect: `endpoint`, `mapping`,
...
- **WHEN** the MCP tool list is enumerated
- **THEN** exactly 16 openconnector tools exist (8 schemas x `search` + `get`)
...
### Requirement: REQ-MCP-103 — OpenConnector MUST NOT expose any MCP write verb on any schema, because its objects are the integration control plane

No `create`, `update`, or `delete` verb MUST be declared on any openconnector schema. These
...
or destroys forensic history. Because OpenConnector ingests data from remote systems it does not
...
#### Scenario: no openconnector write tool is derivable
- **WHEN** the MCP tool list is enumerated
- **THEN** no `openconnector.*.create`, `openconnector.*.update`, or `openconnector.*.delete` tool exists
- **AND** no openconnector tool declares `destructiveHint: true` or a write `scope`
...
- **WHEN** it searches the tool registry for an openconnector write tool
...
structural: openconnector's secret masking is implemented in its **controllers** (e.g.
`EventsController::redactSubscription()`), and an OpenRegister-derived MCP tool reads the stored
object **without passing through any openconnector controller** — so controller-side redaction
...
- **WHEN** the MCP tool list is enumerated
- **THEN** no `openconnector.source.*`, `openconnector.consumer.*`, `openconnector.rule.*`, `openconnector.event_subscription.*`, `openconnector.lti_platform.*`, `openconnector.lti_tool.*`, or `openconnector.eudi_*.*` tool exists
- **AND** no plaintext password, API key, OAuth secret, JWT, signing key, or webhook signing secret is reachable through any openconnector tool
...
- **WHEN** an agent calls `openconnector.call_log.get`
