...
- [x] 1.1 Add `configuration["x-openregister-mcp"]` to the **8** schemas in design.md D1
...
- [x] 1.2 For each: `enabled: true`; `tools` containing **only** `search` and `get`; each verb
...
- [x] 1.3 Add `search.filters` exactly as listed in the design.md D1 table. Every entry MUST be a
...
- [x] 1.5 Confirm **no** dialect block was added to any credential-bearing schema — `source`,
...
- [x] 2.1 `python3 -m json.tool lib/Settings/openconnector_register.json` — valid JSON, and diff
      to confirm the only additions are the 8 `configuration` keys.
- [x] 2.2 Assert every declared filter is a real property: for each of the 8 schemas, diff the
      `filters` list against that schema's `properties` keys. Zero unknown filters.
- [x] 2.3 Re-import the register (`lib/Repair/InitializeRegister.php` / `occ`) and confirm it
      succeeds — `McpAnnotationValidator` gets its chance to reject a bad dialect.
- [x] 2.4 Grep the register for `x-openregister-mcp` and confirm exactly 8 occurrences, on
      exactly the 8 curated slugs.

## 3. Close out

- [x] 3.1 `openspec validate openconnector-mcp-adoption --type change --strict`.
- [x] 3.2 CHANGELOG entry under Unreleased/Added: read-only MCP tool surface (ADR-063) — 8
