...
### Task 1: `IntegriqAgentTools` with six `#[McpTool]` methods + scannable-services registration
...
- **files**: `lib/Mcp/IntegriqAgentTools.php` (new), `lib/Mcp/IntegriqScannableServices.php` (new), `lib/Mcp/McpArgumentValidator.php` (new, decidesk pattern), `lib/AppInfo/Application.php`
- **acceptance_criteria**:
  - GIVEN the app boots WHEN the container resolves `IMcpScannableServices::integriq` THEN it returns `[IntegriqAgentTools::class]` and no `IMcpToolProvider::integriq` alias exists
  - GIVEN the tool catalogue WHEN enumerated THEN it contains exactly 22 integriq tools: 16 derived reads + `runSynchronization`, `testSynchronization`, `testSource`, `replayDeadLetters`, `discardDeadLetters`, `listDeadLetters`, all 2-segment ids
...
- **files**: `lib/actions.seed.json`, `lib/Mcp/IntegriqAgentTools.php`, `tests/Unit/Mcp/`
- **acceptance_criteria**:
...
- **files**: `lib/Mcp/IntegriqAgentTools.php`, `lib/Service/` (staged-proposal handling), `tests/Unit/Mcp/`
- **acceptance_criteria**:
...
- **files**: `lib/Mcp/IntegriqAgentTools.php`, `tests/Unit/Mcp/`
- **acceptance_criteria**:
...
- **acceptance_criteria**:
  - GIVEN Hermiq enumerates the integriq tools WHEN the six are classified THEN run/replay/discard are default-denied writes and run/test/replay show reach `external` (verified once against a live Hermiq, recorded in the change)
