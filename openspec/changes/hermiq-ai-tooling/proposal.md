...
- [x] Project: `integriq` — new `lib/Mcp/IntegriqAgentTools.php` (`#[McpTool]` action methods), `lib/Mcp/IntegriqScannableServices.php`, `lib/Mcp/McpArgumentValidator.php`, action-matrix rows for the two dead-letter actions, tests. openregister (scanner) and hermiq (governance) are consumed unchanged.

## Scope

### In Scope

- Six curated tools on `IntegriqAgentTools`, discovered via `IMcpScannableServices::integriq`:
  - `integriq.runSynchronization` — scope `update`, reach `external`, **approval-gated**; delegates to the `synchronization.run` path (including the sync-safety guardrails: fetch-completeness gate, deletion-ratio guard; `forceDeletion` is NOT exposed as a tool argument)
  - `integriq.testSynchronization` — scope `read`, reach `external` (dry-run, no-write by the test-run guarantee, but it calls the remote source), confirm-classified, not approval-gated
  - `integriq.testSource` — scope `read`, reach `external`; delegates to the `source.test` path
  - `integriq.replayDeadLetters` — scope `update`, reach `external`, **approval-gated**, batch-shaped; delegates to the audited replay paths (REQ-DLR-003/REQ-DLR-009)
  - `integriq.discardDeadLetters` — scope `delete`, reach `instance`, `destructiveHint: true`, **approval-gated**, batch-shaped; delegates to the audited discard paths (a discard is a terminal state, REQ-DLR-004)
  - `integriq.listDeadLetters` — scope `read`, reach `instance`; payload-free projection over both dead-letter stores
