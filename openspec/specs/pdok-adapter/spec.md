...
integriq MUST ship a `PdokConnector` PHP class at `lib/Connectors/PdokConnector.php`
that implements integriq's existing connector contract interface. The class MUST
...
- GIVEN the integriq codebase has been searched for existing PDOK or geo connectors
...
The integriq PDOK adapter SHALL expose four GET routes registered in
...
- GIVEN integriq is installed and a user is authenticated in Nextcloud
...
- WHEN integriq processes the request
- THEN integriq SHALL return HTTP 401 via the standard Nextcloud `requireLogin()` guard
- AND no PDOK upstream call SHALL be made

#### Scenario: Missing required parameter returns 400

- GIVEN the integriq PDOK adapter is installed
- WHEN a frontend sends `GET /api/pdok/suggest` with no `q` parameter
- THEN integriq SHALL return HTTP 400 with `message_key: "pdok.error.missing_query"`

### Requirement: Response Transformation to Canonical PostalAddress Shape

The integriq PDOK adapter MUST implement a `normalize(array $pdokDoc): array` method
...
The integriq PDOK adapter SHALL cache upstream PDOK responses in Nextcloud's
...
The integriq PDOK adapter SHALL upsert successfully fetched and normalized addresses
...
- WHEN integriq successfully fetches and normalizes that address from PDOK
- THEN integriq SHALL create a new OR object in the `addresses` register with
...
- WHEN integriq receives the lookup request
- THEN integriq SHALL return the OR object without calling PDOK upstream
- AND SHALL re-populate the APCu entry

### Requirement: Rate-Limit Handling with Exponential Backoff

The integriq PDOK adapter SHALL implement exponential backoff when PDOK returns
...
- THEN integriq SHALL return HTTP 503 with `message_key: "pdok.unavailable"`
- AND the circuit breaker failure counter SHALL be incremented

### Requirement: Circuit Breaker

The integriq PDOK adapter MUST implement a circuit breaker stored in APCu under
...
- AND integriq SHALL return HTTP 503 with `message_key: "pdok.unavailable"`
...
The integriq PDOK adapter SHALL return HTTP 503 with body
...
- THEN integriq SHALL return HTTP 503 with
...
- THEN integriq SHALL return HTTP 200 with the OR address object
- AND the response SHALL include the header `X-Cache-Stale: true`

### Requirement: Observability

The integriq PDOK adapter SHALL emit one structured Nextcloud log entry per upstream
