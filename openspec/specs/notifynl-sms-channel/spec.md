...
@e2e exclude backend SMS channel (send, status poll, inbound webhook, JWT signing) — covered by PHPUnit, no browser UI

**OpenSpec changes**
- `notifynl-sms-channel` (done) — introduces the `SmsProviderInterface`
  abstraction (`log`/sandbox + `notifynl` REST bindings), the pure
  `PhoneNumberValidator`, `SmsDispatchService`'s send/poll/callback lifecycle,
  the `sms_message` status schema, and the `NotifyNlController` REST surface
  (`POST /api/notifynl/messages`, `GET /api/notifynl/messages/{id}`,
  `POST /api/notifynl/inbound`).
  Archived at `openspec/changes/archive/2026-07-13-notifynl-sms-channel/`.
...
(`BrokeredCallService::assertScopeGuards()`). See
`openspec/changes/archive/2026-07-13-notifynl-sms-channel/design.md` for the
full analysis. A missing or undecryptable credential MUST fail closed with an
