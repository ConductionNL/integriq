...
## Purpose
Handles Integriq install/upgrade and boot-time wiring with OpenRegister: a repair step imports the app's register descriptor into OR via ConfigurationService on every install and upgrade, and boot registers the SynchronizationContractProvider with OR's IntegrationRegistry so SyncContract leaves render in OR object sidebars. Both paths resolve OR lazily and soft-fail with a warning when OpenRegister is absent or resolution throws, so Integriq can install and boot independently without crashing.
