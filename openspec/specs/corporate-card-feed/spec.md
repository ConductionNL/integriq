...
@e2e exclude backend corporate-card feed (card discovery, scheduled sync, transaction-id idempotency) — covered by PHPUnit, no browser UI

**OpenSpec changes**
- `corporate-card-feed` (done) — introduces the `CardfeedProviderInterface`
  abstraction (`log`/sandbox + generic `rest` bindings), the admin-gated enroll +
  card-discovery endpoint, the `CardfeedSyncJob` scheduled transaction sync
  emitting `nl.conduction.cardfeed.transactions.synced` with a `batchUri`, and the
  transaction-id idempotency (`seenTransactionIds`) that makes a replayed sync
  persist no batch and emit no event.
  Archived at `openspec/changes/archive/2026-07-14-corporate-card-feed/`.
